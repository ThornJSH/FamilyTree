...
class MainWindow(QMainWindow):
    ...
    def start_new_tree(self):
        """새 가계도 시작"""
        print("DEBUG: start_new_tree called")
        try:
            ...
            print(f"DEBUG: Inputs - Name: {name}, Tree: {tree_name}")
            ...
            # 중심 인물 생성 (화면 중앙)
            print("DEBUG: Creating Person object...")
            person = Person(
    ...
    def setup_relationship(self, new_person: Person, center_person: Person, rel_type: str):
        ...
        if rel_type in ['spouse', 'cohabitant', 'divorce', 'separation']:
            ...
            # 부모 추가
            other_parent = self.find_person_by_id(center_person.parentId) if center_person.parentId else None
            if other_parent:
                # 두 번째 부모는 첫 번째 부모의 배우자로
                new_person.spouseId = other_parent.id
                other_parent.spouseId = new_person.id
                new_person.relationshipType = 'spouse'
                other_parent.relationshipType = 'spouse'
                new_person.x = other_parent.x + NODE_WIDTH + SIBLING_SPACING
                new_person.y = other_parent.y
            else:
                # 첫 부모
            ...
            if not center_person.parentId:
                QMessageBox.warning(self, "관계 오류", "다태아를 추가하려면, 부모가 있는 형제를 기준 인물로 선택해야 합니다.")
                return
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() == Qt.Key.Key_Delete:
            ...
            if self.initial_client:
                if self.canvas.delete_selected_person(self.initial_client.id):
                    self.save_state_for_undo()
                    self.update_center_person_select()
                    self.update_status("선택한 인물을 삭제했습니다.")
                else:
                    self.update_status("중심인물은 삭제할 수 없습니다.")
