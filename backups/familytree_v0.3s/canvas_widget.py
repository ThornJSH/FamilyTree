...
class CanvasWidget(QGraphicsView):
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.panning:
            ...
            # Ctrl 키(간격 조정)가 눌려있으면 X축으로만 이동 강제 (형제 간격은 보통 가로니까)
            # 단, Shift 키도 같이 누르면 자유 이동 허용 (필요시)
            is_spacing_mode = event.modifiers() & Qt.KeyboardModifier.ControlModifier
            
            for node in self.dragged_group:
                start_pos = self.drag_start_node_pos[node.person.id]
                new_x = start_pos.x()
                new_y = start_pos.y()
                
                if is_spacing_mode:
                    # 간격 조정 모드: 선택된 노드만 이동 (보통 수평 이동)
                    if self.current_drag_axis == 'x' or abs(dx) > abs(dy):
                        new_x += dx
                    # 수직 이동은 허용하지 않음 (간격 조정은 같은 레벨에서 하므로)
                else:
                    # 일반 이동: 그룹 전체 이동, 축 고정 적용
                ...
                # Person 객체 업데이트는 itemChange에서 처리됨
