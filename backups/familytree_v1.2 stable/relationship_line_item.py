...
class RelationshipLineItem(QGraphicsPathItem):
    ...
    def create_handles(self):
        ...
        self.end_handle.setPen(QPen(QColor("#000000"), 2))
        self.end_handle.setFlag(QGraphicsItem.GraphicsItemFlag.ItemIsMovable, False)
        self.end_handle.setZValue(1001)
        self.end_handle.setVisible(False)  # 기본적으로 숨김
    ...
    def update_line(self):
        ...
        # 펜 설정
        pen = QPen(color, 1.5)  # 가계도 선과 동일한 두께
    ...
    def create_zigzag_path(self, x1, y1, x2, y2):
        ...
        for i in range(num_waves * 2 + 1): # 반파장 단위로 계산
            ...
            # 지그재그 오프셋 (반파장마다 위/아래 교차)
            # i=0(0):중앙, i=1(0.5):위, i=2(1.0):중앙, i=3(1.5):아래, i=4(2.0):중앙
    ...
    def mousePressEvent(self, event):
        ...
        # 드래그 핸들 결정 로직
        # 1. 명시적으로 핸들 근처(20px)를 클릭한 경우
        # 2. Ctrl 키를 누른 경우 (가까운 쪽 끝점을 선택)
        
        is_start_target = start_dist < 20
        is_end_target = end_dist < 20
    ...
    def mouseMoveEvent(self, event):
        ...
        if self.dragging_handle == 'start':
            # 시작점 핸들 드래그
            pos = event.scenePos()
            self.relationship_line.x1 = pos.x()
            self.relationship_line.y1 = pos.y()
            self.update_line()
            self.update_handle_positions()
            event.accept()
            ...
            # 끝점 핸들 드래그
            pos = event.scenePos()
            self.relationship_line.x2 = pos.x()
            self.relationship_line.y2 = pos.y()
            self.update_line()
            self.update_handle_positions()
            event.accept()
        else:
    ...
    def itemChange(self, change, value):
        ...
        if change == QGraphicsItem.GraphicsItemChange.ItemSelectedHasChanged:
            ...
            self.relationship_line.x1 += offset.x()
            self.relationship_line.y1 += offset.y()
            self.relationship_line.x2 += offset.x()
            self.relationship_line.y2 += offset.y()
            self.update_handle_positions()
